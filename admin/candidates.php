...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add' || $action === 'edit') {
            ...
            if ($action === 'add') {
                // $stmt = $conn->prepare("INSERT INTO candidates (full_name, position, partylist_id, photo, bio, platform, year_level, course) VALUES (?,?,?,?,?,?,?,?)");
                // $stmt->bind_param("ssississs", $full_name, $position, $partylist, $photo, $bio, $platform, $year_level, $course);
                // // fix: partylist can be null
                $stmt = $conn->prepare("INSERT INTO candidates (full_name, position, partylist_id, photo, bio, platform, year_level, course) VALUES (?,?,?,?,?,?,?,?)");
                $stmt->bind_param("ssisssss", $full_name, $position, $partylist, $photo, $bio, $platform, $year_level, $course);
                $stmt->execute();
                $message = "Candidate added successfully."; $msg_type = 'success';
            } else {
                $id = (int)$_POST['id'];
                $stmt = $conn->prepare("UPDATE candidates SET full_name=?, position=?, partylist_id=?, photo=?, bio=?, platform=?, year_level=?, course=? WHERE id=?");
                $stmt->bind_param("ssisssssi", $full_name, $position, $partylist, $photo, $bio, $platform, $year_level, $course, $id);
                $stmt->execute();
        ...
        <form method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="input-row">
                <div class="form-group" style="flex:2;">
                    ...
                    <div class="input-wrap"><input type="text" name="full_name" required
                        value="<?= htmlspecialchars($edit_cand['full_name'] ?? '') ?>"></div>
                </div>
                <div class="form-group" style="flex:1;">
                    <label class="form-label">Position *</label>
                    <div class="input-wrap">
                        <select name="position" required style="border:none;outline:none;background:transparent;width:100%;padding:12px 0;">
                            <option value="">— Select —</option>
                            <?php foreach (['President','Vice President','Senator','Representative','Governor','Vice Governor'] as $pos): ?>
                            <option value="<?= $pos ?>" <?= ($edit_cand['position'] ?? '') === $pos ? 'selected' : '' ?>><?= $pos ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
            ...
            <div class="input-row">
                <div class="form-group">
                    ...
                    <div class="input-wrap">
                        <select name="partylist_id" style="border:none;outline:none;background:transparent;width:100%;padding:12px 0;">
                            <option value="">— None / Independent —</option>
                            <?php foreach ($pl_options as $pl): ?>
                            <option value="<?= $pl['id'] ?>" <?= ($edit_cand['partylist_id'] ?? 0) == $pl['id'] ? 'selected' : '' ?>>
            ...
            <div class="input-row">
                <div class="form-group">
                    ...
                    <div class="input-wrap"><input type="text" name="year_level" placeholder="e.g. 3rd Year"
                        value="<?= htmlspecialchars($edit_cand['year_level'] ?? '') ?>"></div>
                </div>
                <div class="form-group">
                    <label class="form-label">Course</label>
                    <div class="input-wrap"><input type="text" name="course" placeholder="e.g. BS Computer Science"
                        value="<?= htmlspecialchars($edit_cand['course'] ?? '') ?>"></div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="input-wrap" style="align-items:flex-start;padding:12px 14px;">
                    <textarea name="bio" rows="3" style="border:none;outline:none;background:transparent;width:100%;"><?= htmlspecialchars($edit_cand['bio'] ?? '') ?></textarea>
                </div>
            </div>
            <div class="form-group">
                <label class="form-label">Platform & Advocacies</label>
                <div class="input-wrap" style="align-items:flex-start;padding:12px 14px;">
                    <textarea name="platform" rows="3" style="border:none;outline:none;background:transparent;width:100%;"><?= htmlspecialchars($edit_cand['platform'] ?? '') ?></textarea>
                </div>
            </div>
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <?php if ($c['photo']): ?>
                                <img src="../<?= htmlspecialchars($c['photo']) ?>" style="width:40px;height:40px;object-fit:cover;border-radius:50%;">
                            <?php else: ?>
